...
library CBOR {
  ...
  function encodeType(
    Buffer_Chainlink.buffer memory buf,
    uint8 major,
    uint256 value
  ) private pure {
    if (value <= 23) {
  ...
  function encodeUInt(Buffer_Chainlink.buffer memory buf, uint256 value)
    internal
    pure
  {
  ...
  function encodeInt(Buffer_Chainlink.buffer memory buf, int256 value)
    internal
    pure
  {
    if (value >= 0) {
  ...
  function encodeBytes(Buffer_Chainlink.buffer memory buf, bytes memory value)
    internal
    pure
  {
  ...
  function encodeString(Buffer_Chainlink.buffer memory buf, string memory value)
    internal
    pure
  {
...
contract ChainlinkClient {
  ...
  function buildChainlinkRequest(
    ...
    return
      req.initialize(_specId, _callbackAddress, _callbackFunctionSignature);
  }
